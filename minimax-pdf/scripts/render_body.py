...
def register_fonts(tokens: dict):
    ...
    Values may be a path string (TrueType) or a dict
    {"path": ..., "subfont": n} for TrueType Collection (.ttc) files.
    """
    for name, fpath in tokens.get("font_paths", {}).items():
        if isinstance(fpath, dict):
            path = fpath.get("path", "")
            subfont = fpath.get("subfont", 0)
        else:
            path, subfont = fpath, 0
...
class BeautifulDoc(BaseDocTemplate):
    ...
    def _decorate(self, canv, doc):
        ...
        # Header: title (left) + date (right) — no rule line
        ...
        # Footer: author (left) + page number (right) — no rule line
        canv.setFillColor(HexColor(t["muted"]))
        canv.setFont(t["font_body_rl"], t["size_meta"])
...
def make_styles(t: dict) -> dict:
    ...
    return {
        ...
        "body": ParagraphStyle("Body",
            ...
            spaceAfter=t["para_gap"], alignment=TA_LEFT,
        ),
...
def _divider(accent: str) -> HRFlowable:
    # Slim hairline divider — kept subtle; callers may skip entirely.
    return HRFlowable(
...
def _add_bibliography(story: list, item: dict, ctx: dict):
    ...
    if heading:
        story.append(Paragraph(heading, ctx["styles"]["bib_title"]))
